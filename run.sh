...
general_help() {
    case "$1" in
        --build|-b|build|b)
            pc "green" "$0 --build "
            pc "none" "build all containers, "
            pc "green" "$0 --build <container-name> "
            pc "none" "to build one container by name\n"
        ;;
        --run|-r|run|r)
            pc "green" "$0 --run "
            pc "none" "run all containers, "
            pc "green" "$0 --run <container-name> "
            pc "none" "to run one container by name\n"
        ;;
        --down|-d|down|d)
            pc "green" "$0 --down "
            pc "none" "stop all containers, "
            pc "green" "$0 --down <container-name> "
            pc "none" "to stop one container by name\n"
        ;;
        --own|-o|own|o)
            pc "green" "$0 --own <container-name> <cmd> "
            pc "none" "execute the bash command in the container\n"
        ;;
        --remove|remove)
            pc "green" "$0 --remove "
            pc "none" "remove all containers on your system\n"
        ;;
        wb|wasm-build)
            pc "green" "$0 --wasm-build | $0 -wb "
            pc "none" "build a project in docker environment\n"
        ;;
        *)
            if [ $1 ]; then
